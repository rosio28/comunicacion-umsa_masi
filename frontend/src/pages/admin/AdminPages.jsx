        ...
        <>
          <input type="file" ref={fileRef} accept="image/*" className="hidden"
            onChange={e => {
              ...
              setPreview(URL.createObjectURL(f)); onFile?.(f)
            }}/>
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-2 flex-wrap items-center">
        ...
        {pdfFile && (
          <button type="button" onClick={() => { setPdfFile(null); onFile?.(null) }}
...
export function NoticiaFormPage() {
  ...
  useEffect(() => {
    if (!isEdit || !noticiaData) return
    reset({
      titulo:       noticiaData.titulo       || '',
      resumen:      noticiaData.resumen      || '',
      contenido:    noticiaData.contenido    || '',
      categoria_id: noticiaData.categoria_id ? String(noticiaData.categoria_id) : '',
      destacado:    parseBool(noticiaData.destacado),
      publicado:    parseBool(noticiaData.publicado),
    })
    setImgUrl(noticiaData.imagen_url || '')
  }, [noticiaData, isEdit, reset])
          ...
          {list.map(d => (
            <div key={d.id} className="card p-4 flex items-start gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <button onClick={() => setConfirmId(d.id)} className="btn btn-ghost btn-sm p-1.5 text-red-500"><Trash2 size={14}/></button>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div><label className="label">Tipo</label>
              <select className="input" {...register('tipo')}>
                <option value="titular">Titular</option><option value="interino">Interino</option><option value="invitado">Invitado</option>
              </select>
            </div>
          </div>
            ...
            <div key={c.id} className="card p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex gap-2 flex-shrink-0">
                  ...
                  <button onClick={() => setConfirmId(c.id)} className="btn btn-ghost btn-sm p-1.5 text-red-500"><Trash2 size={14}/></button>
                </div>
              </div>
            </div>
          ))}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing?'Editar convocatoria':'Nueva convocatoria'} size="lg">
        <form onSubmit={handleSubmit(d => save.mutate(d))} className="space-y-3">
          ...
          <FileField label="Documento PDF / Enlace" currentUrl={pdfUrl}
            ...
            onUrl={u  => { setPdfUrl(u);  setPdfFile(null) }}/>
          <div className="flex items-center gap-2"><input type="checkbox" id="pub_c" {...register('publicado')}/><label htmlFor="pub_c" className="text-sm">Publicar</label></div>
          ...
          {list.map(t => (
            <div key={t.id} className="card p-4 flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                {t.archivo_url && <a href={t.archivo_url} target="_blank" rel="noreferrer" className="text-xs text-primary hover:underline">Ver formulario</a>}
              </div>
              <div className="flex gap-2 flex-shrink-0">
                <button onClick={() => openEdit(t)} className="btn btn-ghost btn-sm p-1.5"><Pencil size={14}/></button>
                <button onClick={() => setConfirmId(t.id)} className="btn btn-ghost btn-sm p-1.5 text-red-500"><Trash2 size={14}/></button>
              </div>
            </div>
          ))}
        </div>
              ...
              <div className="p-3">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => openEditAlbum(a)} className="btn btn-ghost btn-sm p-1"><Pencil size={13}/></button>
                    <button onClick={() => setConfirmAlbum(a.id)} className="btn btn-ghost btn-sm p-1 text-red-500"><Trash2 size={13}/></button>
                  </div>
                </div>
              </div>
            </div>
      ...
      <Modal open={!!confirmEliminar} onClose={() => setConfirmEliminar(null)} title="Eliminar usuario permanentemente" size="sm">
        <div className="space-y-4">
          <div className="bg-red-50 border border-red-200 rounded-xl p-4">
            ...
            <p className="text-red-600 text-xs">Se eliminará la cuenta de <strong>{confirmEliminar?.nombre}</strong> ({confirmEliminar?.email}).</p>
          </div>
          <div className="flex gap-2">
