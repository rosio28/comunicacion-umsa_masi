...
// ─── HELPER: parsea booleanos que pueden venir como string desde la BD ───
...
function EstadoBadge({ publicado, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      title={pub ? 'Publicado — clic para despublicar' : 'Borrador — clic para publicar'}
...
function DestacadoBadge({ destacado, onClick }) {
  // FIX: parsear correctamente el booleano (puede venir como string "false" desde BD)
  const dest = parseBool(destacado)
  return (
    <button
      onClick={onClick}
      title={dest ? 'Destacado — clic para quitar' : 'Sin destacar — clic para destacar'}
...
function filtrar(list, filtro) {
  if (filtro === 'publicado') return list.filter(x => parseBool(x.publicado))
  if (filtro === 'borrador')  return list.filter(x => !parseBool(x.publicado))
  if (filtro === 'destacado') return list.filter(x => parseBool(x.destacado))
  return list
}
...
export function AdminNoticiasPage() {
  ...
  // FIX: toggle destacado — enviar booleano real, no el opuesto de un string
                  ...
                  <td className="text-right pr-4">
                    <div className="flex gap-2 justify-end">
                      ...
                      <button
                        ...
                        className="btn btn-ghost btn-sm p-1.5 text-red-500"
                        title="Eliminar"
                      >
                        <Trash2 size={14} />
                      </button>
...
export function NoticiaFormPage() {
  ...
  const { register, handleSubmit, reset, setValue, watch, formState: { errors, isSubmitting } } = useForm({
    defaultValues: {
      ...
      categoria_id: '', destacado: false, publicado: false,
    },
  })
  ...
  // FIX: cargar la noticia individual por slug (la lista no siempre tiene el contenido completo)
  // Usamos el endpoint de lista con limit alto y filtramos — igual que antes pero forzamos reset
  const { data: listaData } = useQuery({
    queryKey: ['noticias-admin-all'],
    queryFn: () => noticiasService.getAll({ limit: 500 }),
    enabled: isEdit,
  })

  useEffect(() => {
    if (!isEdit || !listaData) return
    const nd = listaData?.data?.data?.find(n => String(n.id) === id)
    if (!nd) return

    // FIX: resetear TODOS los campos incluyendo booleanos correctamente
    reset({
      titulo:       nd.titulo       || '',
      resumen:      nd.resumen      || '',
      contenido:    nd.contenido    || '',
      categoria_id: nd.categoria_id ? String(nd.categoria_id) : '',
      destacado:    parseBool(nd.destacado),
      publicado:    parseBool(nd.publicado),
    })
    // FIX: cargar imagen actual para que se muestre en el preview
    if (nd.imagen_url) {
      setImgUrl(nd.imagen_url)
    }
  }, [isEdit, listaData, id, reset])

  const onSubmit = async d => {
    try {
      // FIX: construir FormData manualmente con booleanos correctos
      const fd = new FormData()
      fd.append('titulo',    d.titulo.trim())
      fd.append('contenido', d.contenido.trim())
      if (d.resumen)     fd.append('resumen',     d.resumen.trim())
      ...
      // FIX: imagen — priorizar archivo subido, luego URL
      if (imgFile instanceof File) {
        fd.append('imagen', imgFile)
      } else if (imgUrl && imgUrl.trim()) {
        fd.append('imagen_url', imgUrl.trim())
      }

      const cfg = { headers: { 'Content-Type': 'multipart/form-data' } }
      if (isEdit) {
        await api.put(`/noticias/${id}`, fd, cfg)
        toast.success('Noticia actualizada')
      } else {
        await api.post('/noticias', fd, cfg)
        toast.success('Noticia creada')
      }

      qc.invalidateQueries(['noticias-admin'])
      qc.invalidateQueries(['noticias-admin-all'])
  ...
  return (
    <div className="max-w-3xl">
      <SectionHeader title={isEdit ? 'Editar noticia' : 'Nueva noticia'}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="card p-6 space-y-4">
        ...
        <div>
          ...
          <input className="input" {...register('titulo', { required: 'El título es requerido' })} />
        ...
        <div className="flex gap-3">
          ...
          <Link to="/admin/noticias" className="btn btn-ghost">Cancelar</Link>
        </div>
      </form>
    </div>
  )
...
export function AdminDocentesPage() {
  ...
  const save = useMutation({
    mutationFn: async d => {
      ...
      else if (imgUrl) fd.append('foto_url', imgUrl)
      const cfg = { headers: { 'Content-Type': 'multipart/form-data' } }
      return editing ? api.put(`/docentes/${editing.id}`, fd, cfg) : api.post('/docentes', fd, cfg)
    },
  ...
  return (
    <div>
      <SectionHeader title="Docentes" subtitle={`${list.length} activos`}>
        <button onClick={openNew} className="btn btn-primary btn-sm"><Plus size={15} /> Agregar docente</button>
      </SectionHeader>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Editar docente' : 'Nuevo docente'} size="lg">
        <form onSubmit={handleSubmit(d => save.mutate(d))} className="space-y-3">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div><label className="label">Título (M.Sc., Lic.)</label><input className="input" {...register('titulo_academico')} /></div>
...
export function AdminAlumnosPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['al-admin'],
    queryFn: () => api.get('/mejores-alumnos?all=1').then(r => r.data),
  })
  const all  = data?.data || []
  const list = filtrar(all, filtro)
  ...
  const openEdit = a => {
    ...
    reset({
      ...
      gestion:         a.gestion         || '',
      logros:          a.logros          || '',
      publicado:       parseBool(a.publicado),
    })
    setModal(true)
  }
  const openNew = () => {
  ...
  const save = useMutation({
    mutationFn: async d => {
      ...
      const cfg = { headers: { 'Content-Type': 'multipart/form-data' } }
      return editing ? api.put(`/mejores-alumnos/${editing.id}`, fd, cfg) : api.post('/mejores-alumnos', fd, cfg)
    },
...
export function AdminMultimediaPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['mm-admin'],
    queryFn: () => api.get('/multimedia?all=1').then(r => r.data),
  })
  const all  = data?.data || []
  const list = filtrar(all, filtro)
  ...
  const openEdit = m => {
    setEditing(m)
    setThumbFile(null)
    setThumbUrl(m.thumbnail_url || '')
    reset({
      titulo:        m.titulo        || '',
      tipo:          m.tipo          || 'video',
      autor_nombre:  m.autor_nombre  || '',
      url_contenido: m.url_contenido || '',
      materia_origen:m.materia_origen|| '',
      gestion:       m.gestion       || '',
      descripcion:   m.descripcion   || '',
      destacado:     parseBool(m.destacado),
      publicado:     parseBool(m.publicado),
    })
    setModal(true)
  }
  const openNew = () => {
  ...
  const save = useMutation({
    mutationFn: async d => {
      ...
      const cfg = { headers: { 'Content-Type': 'multipart/form-data' } }
      return editing ? api.put(`/multimedia/${editing.id}`, fd, cfg) : api.post('/multimedia', fd, cfg)
    },
...
export function AdminWhatsappPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['wa-admin'],
    queryFn: () => api.get('/whatsapp/admin').then(r => r.data),
  })
  const all  = data?.data || []
  ...
  const openEdit = g => {
    setEditing(g)
    reset({ materia_nombre: g.materia_nombre || '', semestre: g.semestre || 1, gestion: g.gestion || '', enlace_wa: g.enlace_wa || '', activo: g.activo })
    setModal(true)
  }
  const openNew = () => {
    setEditing(null)
    reset({ materia_nombre: '', semestre: 1, gestion: '', enlace_wa: '', activo: true })
    setModal(true)
  }
  ...
  return (
    <div>
      <SectionHeader title="Grupos de WhatsApp" subtitle={`${all.length} grupos`}>
...
export function AdminEgresadosPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['eg-admin'],
    queryFn: () => api.get('/egresados?all=1').then(r => r.data),
  })
  const all  = data?.data || []
  const list = filtrar(all, filtro)
  ...
  const openEdit = e => {
    setEditing(e)
    setImgFile(null)
    setImgUrl(e.foto_url || '')
    reset({
      nombre_completo:    e.nombre_completo    || '',
      anio_egreso:        e.anio_egreso        || '',
      ocupacion_actual:   e.ocupacion_actual   || '',
      empresa_institucion:e.empresa_institucion|| '',
      testimonio:         e.testimonio         || '',
      linkedin_url:       e.linkedin_url       || '',
      publicado:          parseBool(e.publicado),
    })
    setModal(true)
  }
  const openNew = () => {
  ...
  const save = useMutation({
    mutationFn: async d => {
      ...
      const cfg = { headers: { 'Content-Type': 'multipart/form-data' } }
      return editing ? api.put(`/egresados/${editing.id}`, fd, cfg) : api.post('/egresados', fd, cfg)
    },
    onSuccess: () => { qc.invalidateQueries(['eg-admin']); toast.success('Guardado'); setModal(false) },
  })

  const pub = useMutation({
    mutationFn: e => api.put(`/egresados/${e.id}`, { publicado: !parseBool(e.publicado) }),
    onSuccess: () => { qc.invalidateQueries(['eg-admin']); toast.success('Estado actualizado') },
  })

  const del = useMutation({
    mutationFn: id => api.delete(`/egresados/${id}`),
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          {list.map(e => (
            <div key={e.id} className="card p-4 flex gap-3">
              <div className="w-12 h-12 rounded-xl overflow-hidden bg-secondary-50 flex-shrink-0 flex items-center justify-center">
                {e.foto_url
                  ? <img src={e.foto_url} alt="" className="w-full h-full object-cover" onError={ev => (ev.target.style.display = 'none')} />
                  : <span className="text-secondary font-bold text-xl">{e.nombre_completo.charAt(0)}</span>
                }
              </div>
              <div className="flex-1 min-w-0">
...
export function AdminEventosPage() {
  ...
  const openNew = () => {
    ...
    setImgFile(null)
    setImgUrl('')
    reset({ titulo: '', tipo: 'otro', color: '#C0392B', fecha_inicio: '', fecha_fin: '', lugar: '', enlace_virtual: '', descripcion: '', publicado: true })
    setModal(true)
  }
  ...
  const save = useMutation({
    mutationFn: async d => {
      ...
      else if (imgUrl) fd.append('imagen_url', imgUrl)
      const cfg = { headers: { 'Content-Type': 'multipart/form-data' } }
      return editing ? api.put(`/eventos/${editing.id}`, fd, cfg) : api.post('/eventos', fd, cfg)
    },
  ...
  return (
    <div>
      <SectionHeader title="Eventos" subtitle="Calendario académico">
...
export function AdminGaleriaPage() {
  ...
  const { data: albData } = useQuery({
    queryKey: ['alb-admin'],
    queryFn: () => api.get('/galeria/albumes?all=1').then(r => r.data),
  })
  ...
  const { data: imgData, isLoading: imgsLoad, refetch: refImg } = useQuery({
    queryKey: ['imgs-admin', selAlbum?.id],
    enabled: !!selAlbum,
  ...
  const openEditAlbum = a => { setEditingAlbum(a); setPortadaUrl(a.portada_url || ''); setPortadaFile(null); resetA({ nombre: a.nombre, descripcion: a.descripcion || '', publicado: parseBool(a.publicado) }); setAlbumModal(true) }
  ...
  const albMut = useMutation({
    mutationFn: async d => {
      ...
      else if (portadaUrl) fd.append('portada_url', portadaUrl)
      const cfg = { headers: { 'Content-Type': 'multipart/form-data' } }
      return editingAlbum ? api.put(`/galeria/albumes/${editingAlbum.id}`, fd, cfg) : api.post('/galeria/albumes', fd, cfg)
    },
  ...
  const delAlbum = useMutation({
    mutationFn: id => api.delete(`/galeria/albumes/${id}`),
  ...
  const uploadFiles = async e => {
    ...
    refImg()
    setUploading(false)
    if (ok) toast.success(`${ok} imagen(es) agregada(s)`)
  }
  ...
  const addUrl = async () => {
    ...
    try {
      const fd = new FormData()
      fd.append('imagen_url', newImgUrl.trim())
      fd.append('album_id', selAlbum.id)
  ...
  return (
    <div>
      <SectionHeader title="Galería fotográfica" subtitle="Álbumes e imágenes">
        <button onClick={openNewAlbum} className="btn btn-primary btn-sm"><Plus size={15} /> Nuevo álbum</button>
      </SectionHeader>
      {!selAlbum ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="p-3">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => openEditAlbum(a)} className="btn btn-ghost btn-sm p-1" title="Editar"><Pencil size={13} /></button>
                    <button onClick={() => setConfirmAlbum(a.id)} className="btn btn-ghost btn-sm p-1 text-red-500" title="Eliminar"><Trash2 size={13} /></button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
            <div className="flex items-center gap-3">
              <button onClick={() => setSelAlbum(null)} className="btn btn-ghost btn-sm">← Álbumes</button>
              <div className="w-px h-4 bg-gray-200" />
              <div>
                <p className="font-semibold text-sm">{albumActual?.nombre}</p>
                {albumActual?.descripcion && <p className="text-xs text-gray-400">{albumActual.descripcion}</p>}
              </div>
            </div>
            <div className="flex gap-2">
...
export function AdminMallaPage() {
  ...
  return (
    <div>
      <SectionHeader title="Malla curricular — Pensum 2023" subtitle={`${materias.length} materias`}>
      ...
      <Modal open={newModal} onClose={() => setNewModal(false)} title="Nueva materia">
        <form onSubmit={hN(d => newMut.mutate(d))} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className="label">Tipo</label><select className="input" {...rN('tipo')}><option value="obligatoria">Obligatoria</option><option value="electiva">Electiva</option><option value="taller">Taller</option></select></div>
          </div>
...
export function AdminInstitucionalPage() {
  ...
  const save = useMutation({
    mutationFn: ({ clave, ...d }) => institucionalService.update(clave, d),
    onSuccess: () => { claves.forEach(c => qc.invalidateQueries(['inst-adm', c])); toast.success('Actualizado'); setEditing(null) },
  })
...
export function AdminConvocatoriasPage() {
  ...
  const openEdit = c => {
    ...
    reset({
      ...
      publicado:    parseBool(c.publicado),
    })
    setModal(true)
  }
  const openNew = () => {
    setEditing(null)
    setPdfFile(null)
    reset({ titulo: '', tipo: 'otro', descripcion: '', fecha_limite: '', archivo_url: '', publicado: true })
    setModal(true)
  }
  ...
  const save = useMutation({
    mutationFn: async d => {
      ...
      const cfg = { headers: { 'Content-Type': 'multipart/form-data' } }
      return editing ? api.put(`/convocatorias/${editing.id}`, fd, cfg) : api.post('/convocatorias', fd, cfg)
    },
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Editar convocatoria' : 'Nueva convocatoria'} size="lg">
        <form onSubmit={handleSubmit(d => save.mutate(d))} className="space-y-3">
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              <button type="button" onClick={() => pdfRef.current?.click()} className="btn btn-ghost btn-sm border border-dashed border-gray-300"><Upload size={13} /> {pdfFile?.name || 'Subir PDF'}</button>
              {editing?.archivo_url && !pdfFile && <a href={editing.archivo_url} target="_blank" rel="noreferrer" className="btn btn-ghost btn-sm text-secondary text-xs">Ver actual</a>}
            </div>
...
export function AdminTramitesPage() {
  ...
  const openEdit = t => {
    setEditing(t)
    setPdfFile(null)
    reset({ nombre: t.nombre || '', descripcion: t.descripcion || '', contacto: t.contacto || '', archivo_url: t.archivo_url || '' })
    setModal(true)
  }
  const openNew = () => {
    setEditing(null)
    setPdfFile(null)
    reset({ nombre: '', descripcion: '', contacto: '', archivo_url: '' })
    setModal(true)
  }
  ...
  const save = useMutation({
    mutationFn: async d => {
      ...
      if (pdfFile) fd.append('archivo', pdfFile)
      const cfg = { headers: { 'Content-Type': 'multipart/form-data' } }
      return editing ? api.put(`/tramites/${editing.id}`, fd, cfg) : api.post('/tramites', fd, cfg)
    },
  ...
  return (
    <div>
      <SectionHeader title="Trámites académicos" subtitle={`${list.length} trámites`}>
        <button onClick={openNew} className="btn btn-primary btn-sm"><Plus size={15} /> Agregar trámite</button>
      </SectionHeader>
      {isLoading ? <LoadingCenter /> : (
        <div className="space-y-3">
          ...
          {list.map(t => (
            <div key={t.id} className="card p-4 flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                {t.archivo_url && <a href={t.archivo_url} target="_blank" rel="noreferrer" download className="text-xs text-primary hover:underline">Descargar formulario</a>}
              </div>
              <div className="flex gap-2 flex-shrink-0">
...
export function AdminUsuariosPage() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: res => { qc.invalidateQueries(['usr-admin']); setCreatedPass(res.data?.data?.temp_password); resetC() },
    onError: e => toast.error(e.response?.data?.message || 'Error'),
  })

  const editMut = useMutation({
    mutationFn: ({ id, ...d }) => usuariosService.update(id, d),
    onSuccess: () => { qc.invalidateQueries(['usr-admin']); toast.success('Actualizado'); setEditModal(null) },
  })
  ...
  const openEdit = u => { resetE(u); setEditModal(u) }
  ...
  return (
    <div>
      <SectionHeader title="Gestión de usuarios" subtitle="Solo el superadmin puede gestionar usuarios">
      ...
      {isLoading ? <LoadingCenter /> : (
        <div className="card overflow-hidden">
          <table className="table-pro w-full">
            <thead><tr><th>Usuario</th><th>CI</th><th>Email</th><th>Rol</th><th>Estado</th><th className="text-right pr-4">Acciones</th></tr></thead>
            <tbody>
              {list.map(u => (
                <tr key={u.id}>
                  <td>{u.nombre}</td>
                  ...
                  <td>{u.rol}</td>
                  <td>
                    <button onClick={() => toggleActivoMut.mutate({ id: u.id, activo: !u.activo })} className="badge cursor-pointer text-xs font-semibold">
                  ...
                  <td className="text-right pr-4">
                    <div className="flex justify-end gap-1.5">
                      ...
                      <button onClick={() => setPassModal(u)} className="btn btn-ghost btn-sm">Pass</button>
                      <button onClick={() => setConfirmEliminar(u)}
                        className="btn btn-ghost btn-sm p-1.5 text-red-500" title="Eliminar permanentemente">
                        <Trash2 size={14} />
                      </button>                    </div>
                  </td>
                </tr>
      ...
      <Modal open={!!confirmEliminar} onClose={() => setConfirmEliminar(null)} title="Eliminar usuario permanentemente" size="sm">
        <div className="space-y-4">
          <div className="bg-red-50 border border-red-200 rounded-xl p-4">
            <p className="text-red-700 font-semibold">⚠ Esta acción no se puede deshacer</p>
            <p className="text-red-600 text-sm mt-1">Se eliminará <strong>{confirmEliminar?.nombre}</strong> ({confirmEliminar?.email})</p>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setConfirmEliminar(null)} className="btn btn-ghost flex-1">Cancelar</button>
            <button onClick={() => eliminarMut.mutate(confirmEliminar.id)} disabled={eliminarMut.isPending} className="btn bg-red-600 hover:bg-red-700 text-white flex-1">
      ...
      <Modal open={!!confirmEliminar} onClose={() => setConfirmEliminar(null)}
        title="Eliminar usuario permanentemente" size="sm">
        <div className="space-y-4">
          <div className="bg-red-50 border border-red-200 rounded-xl p-4">
            <p className="text-red-700 font-semibold text-sm mb-1">⚠ Esta acción no se puede deshacer</p>
            <p className="text-red-600 text-xs">
              Se eliminará permanentemente la cuenta de <strong>{confirmEliminar?.nombre}</strong> ({confirmEliminar?.email}).
            </p>
          </div>
          <p className="text-sm text-gray-600">
            Si solo quieres bloquear el acceso, usa el botón de estado (Activo/Inactivo) en la tabla.
          </p>
          <div className="flex gap-2">
            <button onClick={() => setConfirmEliminar(null)}
              className="btn btn-ghost flex-1">Cancelar</button>
            <button onClick={() => eliminarMut.mutate(confirmEliminar.id)}
