...
const NAV = [
  ...
  { to: '/admin/convenios',      icon: Handshake,       label: 'Convenios institucionales', min: 'admin' },
  { section: 'Personas' },
  ...
  const Sidebar = () => (
    <aside className="w-64 bg-secondary flex flex-col h-full">
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-3 space-y-0.5">
        {NAV.map((item, i) => {
          if (item.section) return <p key={i} className="text-xs font-semibold text-blue-400/60 uppercase tracking-widest px-2 pt-4 pb-1 first:pt-2">{item.section}</p>
          ...
          return (
            <NavLink key={item.to} to={item.to} end={item.to === '/admin'}
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all ${isActive ? 'bg-primary text-white shadow-sm' : 'text-blue-100/70 hover:text-white hover:bg-white/10'}`
              }>
              <Icon size={16} className="flex-shrink-0" />{item.label}
            </NavLink>
          )
        })}
      </nav>
      <div className="border-t border-white/10 p-3">
        ...
        <button onClick={handleLogout} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-blue-200/60 hover:text-white hover:bg-white/10 transition-all text-xs font-medium">
          <LogOut size={14} />Cerrar sesión
        </button>
        <Link to="/" className="block text-center text-blue-400/50 hover:text-blue-300 text-xs mt-1 py-1 transition-colors">Ver sitio público</Link>
      </div>
    </aside>
  )
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-gray-100 px-4 sm:px-6 py-3.5 flex items-center gap-3 flex-shrink-0">
          <button onClick={() => setSidebarOpen(true)} className="lg:hidden w-9 h-9 flex items-center justify-center rounded-lg text-gray-500 hover:bg-gray-100">
            <Menu size={20} />
          </button>
          <div className="flex-1" />
          <div className="flex items-center gap-2">
            <span className="hidden sm:inline-block text-xs bg-gray-100 text-gray-600 px-2.5 py-1 rounded-full capitalize font-medium">{usuario?.rol}</span>
            <div className="w-8 h-8 rounded-full bg-primary flex items-center justify-center text-white text-xs font-bold">{usuario?.nombre?.charAt(0)?.toUpperCase()}</div>
          </div>
        </header>
