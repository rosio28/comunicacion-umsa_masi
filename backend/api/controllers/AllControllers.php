<?php
// AllControllers.php — VERSIÓN CORREGIDA COMPLETA
// FIX PRINCIPAL: todos los controllers usan getInputData() en lugar de !empty($_POST)
// Esto resuelve que PUT con multipart/form-data no populaba $_POST.
...
// ─── helper: UPDATE solo campos con valor real ─────────────
...
// ─── helper: parsear booleano de cualquier forma ────────────
function toBool(mixed $v): bool {
    if ($v === null) return false;
    if (is_bool($v)) return $v;
    $s = strtolower(trim((string)$v));
    return in_array($s, ['true','1','yes','t','on'], true);
}
function toBoolStr(mixed $v): string { return toBool($v) ? 'true' : 'false'; }
...
class EventosController {
    ...
    public function store(): void {
        ...
        $db  = Database::getConnection();
        $stmt = $db->prepare(
            "INSERT INTO eventos (titulo,descripcion,tipo,fecha_inicio,fecha_fin,lugar,enlace_virtual,
             color,imagen_url,autor_id,publicado,latitud,longitud)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?) RETURNING id"
        );
        $stmt->execute([
            $titulo, $d['descripcion']??null, $d['tipo']??'otro', $inicio,
            $d['fecha_fin']??null, $d['lugar']??null, $d['enlace_virtual']??null,
            $d['color']??'#C0392B', $img, $payload['id'], $pub,
    ...
    public function update(int $id): void {
        ...
        if (array_key_exists('fecha_fin',$d))      $fields['fecha_fin']      = $d['fecha_fin'] ?: null;
        if (array_key_exists('lugar',$d))           $fields['lugar']          = $d['lugar'];
        if (array_key_exists('enlace_virtual',$d))  $fields['enlace_virtual'] = $d['enlace_virtual'];
        ...
        if (isset($d['latitud'])  && $d['latitud']  !== '')  $fields['latitud']  = $d['latitud'];
        if (isset($d['longitud']) && $d['longitud'] !== '')  $fields['longitud'] = $d['longitud'];
...
class WhatsappController {
    ...
    public function update(int $id): void {
        ...
        if (array_key_exists('activo',$d)) $fields['activo']        = toBoolStr($d['activo']);
        $fields['actualizado_por'] = $payload['id'];
        updateIfNotEmpty($db,'grupos_whatsapp',$id,$fields);
        $db->prepare("UPDATE grupos_whatsapp SET actualizado_en=NOW() WHERE id=?")->execute([$id]);
...
class InstitucionalController {
    ...
    public function update(string $clave): void {
        ...
        $img = $d['imagen_url'] ?? null;
        if (!empty($_FILES['imagen']['tmp_name'])) { $up=uploadImage($_FILES['imagen'],'institucional'); if($up) $img=$up; }

        $db = Database::getConnection();
        $db->prepare("UPDATE contenido_institucional
            SET titulo=COALESCE(NULLIF(?,''),titulo),
                ...
                actualizado_en=NOW()
            WHERE clave=?")
           ->execute([$d['titulo']??null, $d['contenido'], $img, $payload['id'], $clave]);
...
class ContactoController {
    public function enviar(): void {
        ...
        $ok = Mailer::send(
            ADMIN_EMAIL, 'CCS UMSA',
            "Nuevo mensaje web — $nombre",
            "<p><strong>Nombre:</strong> $nombre</p>
             <p><strong>Correo:</strong> $email</p>
             <p><strong>Mensaje:</strong><br>" . nl2br($mensaje) . "</p>"
        );

        // También responder al remitente
        Mailer::send(
            (string)$email, $nombre,
            'Recibimos tu mensaje — CCS UMSA',
            "<p>Hola <strong>$nombre</strong>,</p>
             <p>Hemos recibido tu mensaje y te responderemos a la brevedad.</p>
             <p><em>Este es un correo automático, no respondas a este mensaje.</em></p>"
        );

        if ($ok) {
            Response::success(null, 'Mensaje enviado correctamente');
        } else {
            Response::error('Error al enviar el mensaje. Intenta de nuevo.', 500);
        }
    }
}
...
class UsuariosController {
    ...
    public function store(): void {
        ...
        $inicialN     = strtoupper(mb_substr($nombreNorm, 0, 1, 'UTF-8'));
        $inicialA     = strtoupper(mb_substr($apellidoNorm, 0, 1, 'UTF-8'));
        $tempPass     = trim($d['ci']) . $inicialN . $inicialA;
        ...
        try {
            ...
            $stmt->execute([
                ...
                trim($d['ci']), trim($d['primer_apellido']),
                trim($d['segundo_apellido'] ?? ''), trim($d['telefono'] ?? ''),
            ]);
            $nuevo = $stmt->fetch();
            ...
            Mailer::send(
                ...
                "<p>Hola <strong>{$d['nombre']}</strong>,</p>
                 ...
                 <p style='color:#C0392B;'><strong>⚠ Debes cambiarla al ingresar por primera vez.</strong></p>"
            );
    ...
    public function update(int $id): void {
        ...
        if (array_key_exists('segundo_apellido',$d)) $fields['segundo_apellido'] = trim($d['segundo_apellido']);
        if (array_key_exists('telefono',$d)) $fields['telefono'] = trim($d['telefono']);
        ...
        if (!empty($fields)) {
            ...
            $params[] = $id;
            $db->prepare("UPDATE usuarios SET $sets, actualizado_en = NOW() WHERE id = ?")->execute($params);
        }
    ...
    public function resetPasswordAdmin(int $id): void {
        ...
        if (!$newPass) {
            ...
            $newPass = ($usr['ci'] ?? 'TEMP')
                . strtoupper(mb_substr($nombreNorm, 0, 1, 'UTF-8'))
                . strtoupper(mb_substr($apellidoNorm, 0, 1, 'UTF-8'));
        }
        if (strlen($newPass) < 4) Response::error('Contraseña demasiado corta');

        $hash = password_hash($newPass, PASSWORD_BCRYPT, ['cost' => 12]);
        $db->prepare("UPDATE usuarios SET password_hash=?, debe_cambiar_password=true, actualizado_en=NOW() WHERE id=?")
           ->execute([$hash, $id]);

        Mailer::send(
            $usr['email'], $usr['nombre'],
            'Contraseña restablecida — CCS UMSA',
            "<p>Hola <strong>{$usr['nombre']}</strong>,</p>
             <p>Tu contraseña fue restablecida por el administrador.</p>
             <p><strong>Nueva contraseña temporal:</strong>
    ...
    public function destroy(int $id): void {
        ...
        $d   = getInputData();
        $db  = Database::getConnection();
        if ($id === (int)$payload['id']) Response::error('No puedes eliminar tu propia cuenta', 400);
        $permanente = !empty($d['permanente']) && toBool($d['permanente']);
        if ($permanente) {
...
class MultimediaController {
    public function index(): void {
        ...
        $stmt = $db->prepare("SELECT * FROM multimedia_estudiantes $where ORDER BY creado_en DESC LIMIT 100");
    ...
    public function store(): void {
        ...
        $db->prepare("INSERT INTO multimedia_estudiantes (titulo,tipo,descripcion,url_contenido,thumbnail_url,autor_nombre,materia_origen,gestion,destacado,publicado) VALUES (?,?,?,?,?,?,?,?,?,?)")
           ->execute([$titulo,$d['tipo']??'video',$d['descripcion']??null,$d['url_contenido']??null,$thumb,$autor,$d['materia_origen']??null,$d['gestion']??null,$dest?'true':'false',$pub?'true':'false']);
    ...
    public function update(int $id): void {
        ...
        // Imagen thumbnail
        $thumb = isset($d['thumbnail_url']) && $d['thumbnail_url'] !== '' ? $d['thumbnail_url'] : null;
        if (!empty($_FILES['thumbnail']['tmp_name'])) { $up=uploadImage($_FILES['thumbnail'],'thumbnails'); if($up) $thumb=$up; }
        ...
        if (array_key_exists('materia_origen',$d)) $fields['materia_origen']= $d['materia_origen'];
        if (array_key_exists('gestion',$d))        $fields['gestion']       = $d['gestion'];
        if (!empty($d['url_contenido']))   $fields['url_contenido'] = $d['url_contenido'];
        if (array_key_exists('destacado',$d))      $fields['destacado']     = toBoolStr($d['destacado']);
        if (array_key_exists('publicado',$d))      $fields['publicado']     = toBoolStr($d['publicado']);
        if ($thumb !== null)               $fields['thumbnail_url'] = $thumb;

        if (!empty($fields)) {
            $sets   = implode(', ', array_map(fn($k) => "$k=?", array_keys($fields)));
            $params = array_values($fields);
            $params[] = $id;
            $db->prepare("UPDATE multimedia_estudiantes SET $sets WHERE id=?")->execute($params);
        }
...
class MejoresAlumnosController {
    ...
    public function update(int $id): void {
        ...
        if (!empty($d['gestion']))          $fields['gestion']           = $d['gestion'];
        if (array_key_exists('semestre_actual',$d)) $fields['semestre_actual'] = $d['semestre_actual'] ?: null;
        if (array_key_exists('logros',$d))  $fields['logros']            = $d['logros'];
        if (array_key_exists('publicado',$d)) $fields['publicado']       = toBoolStr($d['publicado']);
        ...
        if (!empty($fields)) {
            ...
            $params = array_values($fields);
            $params[] = $id;
            $db->prepare("UPDATE mejores_estudiantes SET $sets WHERE id=?")->execute($params);
        }
        Response::success(null, 'Actualizado');
    }
    public function destroy(int $id): void {
...
class EgresadosController {
    ...
    public function update(int $id): void {
        ...
        if (array_key_exists('ocupacion_actual',$d))    $fields['ocupacion_actual']    = $d['ocupacion_actual'];
        if (array_key_exists('empresa_institucion',$d)) $fields['empresa_institucion'] = $d['empresa_institucion'];
        if (array_key_exists('testimonio',$d))          $fields['testimonio']          = $d['testimonio'];
        if (array_key_exists('linkedin_url',$d))        $fields['linkedin_url']        = $d['linkedin_url'];
        if (array_key_exists('publicado',$d))           $fields['publicado']           = toBoolStr($d['publicado']);
        ...
        if (!empty($fields)) {
            ...
            $params = array_values($fields);
            $params[] = $id;
            $db->prepare("UPDATE egresados SET $sets WHERE id=?")->execute($params);
        }
        Response::success(null, 'Actualizado');
    }
    public function destroy(int $id): void {
...
class TramitesController {
    ...
    public function update(int $id): void {
        ...
        // Archivo: si viene uno nuevo (file o url), reemplaza; si no hay nada nuevo, mantiene el actual
        $archivo = null;
        if (!empty($_FILES['archivo']['tmp_name'])) {
            $up = uploadFile($_FILES['archivo'], 'tramites');
            if ($up) $archivo = $up;
        } elseif (isset($d['archivo_url']) && $d['archivo_url'] !== '') {
        ...
        if (!empty($d['nombre']))          $fields['nombre']       = $d['nombre'];
        if (array_key_exists('descripcion',$d)) $fields['descripcion'] = $d['descripcion'];
        if (array_key_exists('contacto',$d))    $fields['contacto']    = $d['contacto'];
        ...
        if (!empty($fields)) {
            ...
            $params = array_values($fields);
            $params[] = $id;
            $db->prepare("UPDATE tramites SET $sets WHERE id=?")->execute($params);
        }
...
class ConvocatoriasController {
    ...
    public function update(int $id): void {
        ...
        // Archivo: si viene uno nuevo reemplaza; si no, mantiene el actual
        $archivo = null;
        if (!empty($_FILES['archivo']['tmp_name'])) {
            $up = uploadFile($_FILES['archivo'], 'convocatorias');
            if ($up) $archivo = $up;
        } elseif (isset($d['archivo_url']) && $d['archivo_url'] !== '') {
            $archivo = $d['archivo_url'];
        }
        ...
        if (!empty($fields)) {
            ...
            $params = array_values($fields);
            $params[] = $id;
            $db->prepare("UPDATE convocatorias SET $sets WHERE id=?")->execute($params);
        }
...
class EventoSesionesController {
    ...
    public function store(int $eventoId): void {
        ...
        if (!$num) { $q=$db->prepare("SELECT COALESCE(MAX(numero_sesion),0)+1 FROM evento_sesiones WHERE evento_id=?"); $q->execute([$eventoId]); $num=(int)$q->fetchColumn(); }
        ...
        $stmt->execute([$eventoId,$num,$titulo,$d['fecha']??null,$d['descripcion']??null,$d['contenido_visto']??null,$d['material_url']??null,$d['grabacion_url']??null,$d['enlace_virtual']??null,toBoolStr($d['publicado']??true)]);
    ...
    public function update(int $sesionId): void {
        ...
        $fields = [];
        if (!empty($d['titulo']))          $fields['titulo']          = $d['titulo'];
        if (array_key_exists('fecha',$d))  $fields['fecha']           = $d['fecha'] ?: null;
        if (array_key_exists('descripcion',$d))     $fields['descripcion']     = $d['descripcion'];
        if (array_key_exists('contenido_visto',$d)) $fields['contenido_visto'] = $d['contenido_visto'];
        if (array_key_exists('material_url',$d))    $fields['material_url']    = $d['material_url'];
        if (array_key_exists('grabacion_url',$d))   $fields['grabacion_url']   = $d['grabacion_url'];
        if (array_key_exists('enlace_virtual',$d))  $fields['enlace_virtual']  = $d['enlace_virtual'];
        if (array_key_exists('publicado',$d))       $fields['publicado']       = toBoolStr($d['publicado']);
...
class ConveniosController {
    ...
    public function store(): void {
        ...
        $stmt->execute([$nombre,$d['tipo_institucion']??'empresa',$d['descripcion']??null,$d['tipo_convenio']??'pasantia',isset($d['cupos_disponibles'])&&$d['cupos_disponibles']!==''?(int)$d['cupos_disponibles']:null,isset($d['duracion_meses'])&&$d['duracion_meses']!==''?(int)$d['duracion_meses']:null,$d['requisitos']??null,$d['fecha_inicio']??null,$d['fecha_vencimiento']??null,$d['contacto_nombre']??null,$d['contacto_email']??null,$d['contacto_telefono']??null,$logo,toBoolStr($d['activo']??true),$pub]);
    ...
    public function update(int $id): void {
        ...
        if (array_key_exists('requisitos',$d))     $fields['requisitos']         = $d['requisitos'];
        ...
        if (array_key_exists('contacto_nombre',$d))   $fields['contacto_nombre']   = $d['contacto_nombre'];
        if (array_key_exists('contacto_email',$d))    $fields['contacto_email']    = $d['contacto_email'];
        if (array_key_exists('contacto_telefono',$d)) $fields['contacto_telefono'] = $d['contacto_telefono'];
...
class TransparenciaController {
    public function index(): void {
        $db   = Database::getConnection();
        $all  = !empty($_GET['all']);
        $where = $all ? '' : 'WHERE publicado=true';
        $stmt  = $db->query("SELECT * FROM documentos_transparencia $where ORDER BY publicado_en DESC, creado_en DESC");
        Response::success($stmt->fetchAll());
    }
    public function store(): void {
        $payload = Auth::requireAuth(); Auth::requireRole($payload, ['admin','superadmin']);
    ...
    public function update(int $id): void {
        ...
        if (!empty($fields)) {
            $sets   = implode(', ', array_map(fn($k) => "$k=?", array_keys($fields)));
            $params = array_values($fields); $params[] = $id;
            $db->prepare("UPDATE documentos_transparencia SET $sets WHERE id=?")->execute($params);
        }
        Response::success(null, 'Actualizado');
    }
    public function destroy(int $id): void {
        $payload = Auth::requireAuth(); Auth::requireRole($payload, ['admin','superadmin']);
        Database::getConnection()->prepare("DELETE FROM documentos_transparencia WHERE id=?")->execute([$id]);
        Response::success(null, 'Eliminado');
    }
}
